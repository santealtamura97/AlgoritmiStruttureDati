#ifndef LIST_VECTOR_H_INCLUDED
#define LIST_VECTOR_H_INCLUDED
...
class List_vector : public Linear_list<T, int>{
 public:
	...
	void exchange(position p1,position p2);
	int maxi();
	int mini();
	void move_min_max();
	void insertionSort();
	...
	bool operator!=(const List_vector<T> &) const; // tests two list for disequality
...
template<class T>
void List_vector<T>::exchange(position p1,position p2){
    value_type temp;
    temp=elements_[p1-1];
    elements_[p1-1]=elements_[p2-1];
    elements_[p2-1]=temp;
}

template<class T>
int List_vector<T>::maxi(){
    int max_el=elements_[0];
    for(int i=0;i<length_;i++){
        if(max_el<elements_[i])
            max_el=elements_[i];
    }
    return max_el;
}
...
template<class T>
int List_vector<T>::mini(){
    int min_el=elements_[0];
    for(int i=0;i<length_;i++){
        if(min_el>elements_[i])
            min_el=elements_[i];
    }
    return min_el;
}
...
template<class T>
void List_vector<T>::move_min_max(){
    int min=mini();
    int max=maxi();
    elements_[begin()-1]=min;
    elements_[length_-1]=max;
}

template<class T>
void List_vector<T>::insertionSort(){
        int i,key,j;
        for(i=1;i<length_;i++){
            key=elements_[i];
            j=i-1;
            while(j>=0 && elements_[j]>key){
...
template<class T>
int List_vector<T>::getLenght_(){
    return length_;
}

template<class T>
int List_vector<T>::getArrayDimension(){
