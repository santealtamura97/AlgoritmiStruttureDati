...
template<class T>
void List_vector<T>::reverse() {
	List_vector<T>::position left;
	List_vector<T>::position right;
	...
	List_vector<T>::value_type v;
	while (left != right && right != previous(left)){ //seconda condizione se è dispari la lunghezza
		v = read(left);
